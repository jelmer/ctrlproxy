...
static GIOStatus irssi_gnutls_cert_step(GIOTLSChannel *chan)
{
	...
	{
		case 0:
			return gnutls_verify_peer_certificate(chan->session);
		case GNUTLS_E_AGAIN:
...
GIOChannel *irssi_gnutls_get_iochannel(GIOChannel *handle, gboolean server)
{
	...
	gnutls_transport_set_ptr( chan->session, (gnutls_transport_ptr)sd);
	...
	else if(!(cert = _get_peer_certificate(chan->session)))
	{
		if(!server)
		{
