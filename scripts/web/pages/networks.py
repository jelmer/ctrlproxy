...
class networks(page):
		...
		for (namek, value) in self.handler.post_headers.items():
			...
			if namek == "nick" and net and xml.prop("nick") != value[0]:
				...
				try:
					...
					xml.newChild(None, namek[4:], value[0])
	...
	def view(self, name):
		...
		for n in ('autoconnect','name','nick','username','fullname'):
			c.write(self.tmpl.format("pair_row", title=n.capitalize(), contents=xml.prop(n)))
		title = "Channels"
		...
		self.t.write(self.tmpl.format("a_func", href='/confirm/?msg='+urllib.quote("Really delete Network %s ?" %name)+'&page=/networks/delete/', name="Delete"))
		...
		for s in lines:
			...
			c.write(self.tmpl.format("row", contents=[
									...
									self.tmpl.format("cell_cl", cl=cl, contents=[
										...
										self.tmpl.format("a_func", href='/networks/delserver/%s/%i' %(uname,i), name="Del")
										])
									]))
	...
	def newserver(self, name):
		...
		i = f.open_layer("infos", title="New Server", colspan="4")
		opt = []
	...
	def editserver(self, name, server):
		...
		f = self.t.open_layer("form", action="/networks/saveserver/%s/%s" %(uname, server))
		t = f.open_layer("infos", title="Edit Server", colspan="4")
