<?xml version='1.0'?>
<!--
	Convert CtrlProxy XML documentation to DocBook
	(C) 2004-2007 Jelmer Vernooij
-->
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:template name="subcmd-list">
		<xsl:if test="ctrlproxy-command != ''">
			<xsl:element name="variablelist">
				<xsl:for-each select="ctrlproxy-command">
	...
	<xsl:template match="ctrlproxy-setting">
	...
	<xsl:template match="ctrlproxy-command">
		<xsl:element name="sect1">
			<xsl:call-template name="cmd">
