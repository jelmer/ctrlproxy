...
{
	admin_commands = g_list_insert_sorted(admin_commands, g_memdup2(cmd, sizeof(*cmd)), cmp_cmd);
...
gboolean start_admin_socket(struct global *global)
{
	...
	strncpy(un.sun_path, global->config->admin_socket, sizeof(un.sun_path) - 1);
